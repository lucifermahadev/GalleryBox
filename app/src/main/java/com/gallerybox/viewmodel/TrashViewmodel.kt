...
import com.gallerybox.engine.DocumentFile
...
class TrashViewModel @Inject constructor(
    ...
    fun confirmPendingDocumentTrash(itemsToTrash: List<DocumentFile>, onPermissionRequested: (IntentSenderRequest) -> Unit) = viewModelScope.launch(Dispatchers.IO) {
        if (itemsToTrash.isEmpty()) return@launch
        val mappedEntities = itemsToTrash.map { doc ->
            TrashEntity(
                deletedTimestamp = System.currentTimeMillis(),
                originalPath = doc.uri.toString(),
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            val uris = itemsToTrash.map { it.uri }
            try {
                val intentSender = MediaStore.createTrashRequest(resolver, uris, true).intentSender
                pendingTrashEntities = mappedEntities
                withContext(Dispatchers.Main) {
                    onPermissionRequested(IntentSenderRequest.Builder(intentSender).build())
                }
                ...
                uris.forEach { uri ->
                    try {
                        if (resolver.delete(uri, null, null) > 0) deleteCount++
                    } catch (_: Exception) {}
                }

                if (deleteCount > 0) {
                    galleryDao.insertTrashItemsBulk(mappedEntities)
                }
            ...
            urisFallbackDelete(uris = itemsToTrash.map { it.uri }, entities = mappedEntities) { onRefreshDocuments?.invoke() }
        }
    }

    fun onPermissionResultDocument(granted: Boolean) {
        onPermissionResultGlobal(granted)
        if (granted) {
            viewModelScope.launch { onRefreshDocuments?.invoke() }
        }
    }

    fun moveDocumentsToTrash(documents: List<DocumentFile>) = viewModelScope.launch(Dispatchers.IO) {
        val trashItems = documents.map {
            TrashEntity(
                deletedTimestamp = System.currentTimeMillis(),
                originalPath = it.uri.toString(),
        ...
        withContext(Dispatchers.Main) { onRefreshDocuments?.invoke() }
    }

    fun restoreDocuments(items: List<TrashEntity>, onComplete: (() -> Unit)? = null) = viewModelScope.launch(Dispatchers.IO) {
        val successfulRestores = mutableListOf<Long>()
    ...
    fun permanentlyDeleteDocuments(items: List<TrashEntity>, onComplete: (() -> Unit)? = null) = viewModelScope.launch(Dispatchers.IO) {
        val successfulDeletes = mutableListOf<Long>()
